...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [accountBlock, setAccountBlock] = useState<AuthContextType['accountBlock']>(null);
  ...
  const loadProfile = useCallback(async (authUser: any) => {
    ...
    if (p) {
      ...
      const currentDomain = window.location.hostname;
      if (!(p as any).signup_domain) updates.signup_domain = currentDomain;
      if (!(p as any).role || (p as any).role === 'user') updates.role = 'member';
  ...
  return (
    <AuthContext.Provider value={{
      user, profile, loading, isLoggedIn, isAdmin, signOut, refreshProfile,
    ...
    {isLoggedIn && user && !needsProfileCompletion && (
      <PaymentNudgePopup user={user} siteSlug="ai-hub" />
    )}
